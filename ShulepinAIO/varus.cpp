...
namespace varus
{
	...
	namespace config::auto_dash::r::prediction
	{
		float max_range;
	}

	namespace config::auto_dash::r::settings
	{
		std::unordered_map<std::string, bool> enabled;
		std::unordered_map<std::string, float> my_hp;
		std::unordered_map<std::string, float> enemy_hp;
		std::unordered_map<std::string, float> max_range;
		std::unordered_map<std::string, int> cast_mode;
	}
	...
	void create_menu()
	{
		...
		{
			/*
			const auto farm = config->add_sub_category("shulepin_aio_varus_farm", "Farm Settings");
			{
				//farm->add_separator();
				//farm->add_label("                          |> Farm Settings <|                          ");
				//farm->add_separator();
				
				const auto lane_clear = farm->add_sub_category("shulepin_aio_varus_farm_lane_clear", "Lane Clear");
				{
					lane_clear->add_separator();
					lane_clear->add_label("                          |> Spell Manager <|                          ");
					lane_clear->add_separator();
					lane_clear->add_checkbox("shulepin_aio_varus_farm_lane_clear_use_q", "Use [Q]", true, [](const bool value) {  });
					lane_clear->add_checkbox("shulepin_aio_varus_farm_lane_clear_use_e", "Use [E]", true, [](const bool value) {  });
					lane_clear->add_separator();
					lane_clear->add_label("                          |> Spell Settings <|                          ");
					lane_clear->add_separator();
					const auto lane_clear_q = lane_clear->add_sub_category("shulepin_aio_varus_farm_lane_clear_q", "[Q] Settings");
					{
						
					}
					const auto lane_clear_e = lane_clear->add_sub_category("shulepin_aio_varus_farm_lane_clear_e", "[E] Settings");
					{
						
					}
				}
				const auto jungle_clear = farm->add_sub_category("shulepin_aio_varus_farm_jungle_clear", "Jungle Clear");
				{
			...
			const auto combo = config->add_sub_category("shulepin_aio_varus_combo", "Combo Settings");
			{
				...
				const auto combo_r = combo->add_sub_category("shulepin_aio_varus_combo_r", "[R] Settings");
				{
					const auto combo_r_cast_conditions = combo_r->add_sub_category("shulepin_aio_varus_combo_r_cast_conditions", "Cast Conditions");
					{
						...
						combo_r_cast_conditions->add_checkbox("shulepin_aio_varus_combo_r_cast_conditions_dont_overkill", " ^- Don't overkill", true, [](const bool value) { config::combo::r::cast_conditions::dont_overkill = value; });
			...
			const auto auto_kill = config->add_sub_category("shulepin_aio_varus_auto_kill", "Auto Kill Settings");
			{
				...
				auto_kill->add_checkbox("shulepin_aio_varus_auto_kill_check_recall", "Disable while recall", true, [](const bool value) { config::auto_kill::check_recall = value; });
				auto_kill->add_separator();
			...
			const auto auto_dash = config->add_sub_category("shulepin_aio_varus_auto_dash", "Auto Dash Settings");
			{
				auto_dash->add_separator();
				auto_dash->add_label("                            |> Auto Dash <|                            ");
				auto_dash->add_separator();
				auto_dash->add_checkbox("shulepin_aio_varus_auto_dash_enabled", "Enabled", true, [](const bool value) { config::auto_dash::enabled = value; });
				auto_dash->add_checkbox("shulepin_aio_varus_auto_dash_check_recall", "Disable while recall", true, [](const bool value) { config::auto_dash::check_recall = value; });
				auto_dash->add_separator();
				auto_dash->add_label("                          |> Spell Manager <|                          ");
				auto_dash->add_separator();
				auto_dash->add_checkbox("shulepin_aio_varus_auto_dash_use_r", "Use [R]", true, [](const bool value) { config::auto_dash::use_r = value; });
				auto_dash->add_separator();
				auto_dash->add_label("                          |> Spell Settings <|                          ");
				auto_dash->add_separator();
				const auto auto_dash_r = auto_dash->add_sub_category("shulepin_aio_varus_auto_dash_r", "[R] Settings");
				{
					const auto auto_kill_r_prediction = auto_dash_r->add_sub_category("shulepin_aio_varus_auto_dash_r_prediction", "Prediction Settings");
					{
						auto_kill_r_prediction->add_slider_float("shulepin_aio_varus_auto_dash_q_max_range", "Max range", r->get_range() / 2, r->get_range(), 10.f, r->get_range() - 50.f, [](const float value) {config::auto_dash::r::prediction::max_range = value; });
					}
					const auto auto_dash_r_settings = auto_dash_r->add_sub_category("shulepin_aio_varus_auto_dash_r_settings", "Champion Settings");
					{
						for (const auto& hero : g_sdk->object_manager->get_heroes())
						{
							if (!utils::is_enemy(hero)) continue;
							const auto& char_name = hero->get_char_name();
							std::string hero_sub_category_id = "shulepin_aio_varus_auto_dash_r_settings_" + std::string(char_name);
							const auto hero_sub_category = auto_dash_r_settings->add_sub_category(hero_sub_category_id, std::string(char_name));
							{
								hero_sub_category->add_checkbox(hero_sub_category_id + std::string("_enabled"), "Enabled", true, [char_name](const bool value){ config::auto_dash::r::settings::enabled[char_name] = value; });
								hero_sub_category->add_slider_float(hero_sub_category_id + std::string("_max_range"), "Max range", 0.f, r->get_range(), 10.f, r->get_range(), [char_name](const float value) {config::auto_dash::r::settings::max_range[char_name] = value; });
								hero_sub_category->add_slider_float(hero_sub_category_id + std::string("_my_hp"), "My min. HP (%)", 0.f, 100.f, 1.f, 100.f, [char_name](const float value) {config::auto_dash::r::settings::my_hp[char_name] = value; });
								hero_sub_category->add_slider_float(hero_sub_category_id + std::string("_enemy_hp"), "Enemy min. HP (%)", 0.f, 100.f, 1.f, 100.f, [char_name](const float value) {config::auto_dash::r::settings::enemy_hp[char_name] = value; });
								hero_sub_category->add_combo(hero_sub_category_id + std::string("_cast_mode"), "Cast mode", { "Cast always", "Cast only in combo" }, 1, [char_name](const int value){ config::auto_dash::r::settings::cast_mode[char_name] = value; });
							}
						}
					}
				}
	...
	float calculate_q_damage(game_object* target, const float charge_time, const int collision_count = 0)
	{
		...
		const float damage_reduction_percentage = std::min(0.15f * static_cast<float>(collision_count), 0.67f);
		total_damage *= (1.0f - damage_reduction_percentage);
	...
	void e_combat_logic()
	{
		...
		if (sdk::orbwalker->harass())
		{
			...
			const auto stack_check = get_real_stacks(target) >= config::harass::e::min_stacks || w->get_level() == 0;
			const auto last_cast_check = q->is_issue_order_passed(1.f);
			const auto dist_to_target = player->get_position().distance(target->get_position());
	...
	void r_combat_logic()
	{
		...
		if (sdk::orbwalker->combo())
		{
			...
			{
				if (!q->is_issue_order_passed(1.f))
				{
					return;
				}
				...
				if (target)
				{
					if (config::combo::r::cast_conditions::dont_overkill &&
						calculate_total_damage(target, true, q->is_ready(), w->is_ready(), e->is_ready(), false) >
						utils::get_real_health(target, "ALL"))
					{
						return;
					}
	...
	void q_auto_kill_logic(game_object* target)
	{
		...
		const auto check_q_damage = calculate_q_damage(target, 1.f, collision_count) + check_w_damage;
		...
		const auto real_q_damage = calculate_q_damage(target, q->get_charged_time(1.25f), collision_count) + real_w_damage;
		...
		if (config::auto_kill::check_recall && utils::is_recalling(player))
		{
			return;
		}
	...
	{
		if (!config::auto_dash::enabled ||
			!config::auto_dash::use_r ||
			!r->is_ready() ||
			!utils::is_enemy(target) ||
			!config::auto_dash::r::settings::enabled[target->get_char_name()] ||
			!utils::is_valid_target(target, r->get_range()) ||
			!target->is_dashing() ||
			q->is_charging())
		{
		...
		if (config::auto_dash::check_recall && utils::is_recalling(player))
		{
			return;
		}

		const auto my_hp = config::auto_dash::r::settings::my_hp[target->get_char_name()];
		const auto enemy_hp = config::auto_dash::r::settings::enemy_hp[target->get_char_name()];
		const auto cast_mode = config::auto_dash::r::settings::cast_mode[target->get_char_name()];
		const auto max_range = config::auto_dash::r::settings::max_range[target->get_char_name()];
		const auto check_cast_mode = cast_mode == 1 && sdk::orbwalker->combo() || cast_mode == 0;
		...
		if (pred.is_valid &&
			...
			my_hp >= utils::get_percent_health(player) &&
			enemy_hp >= utils::get_percent_health(target) &&
			check_cast_mode)
		{
			r->cast_spell(end_path);
		}
	}
	...
	void __fastcall present()
	{
		...
		const auto target = utils::get_target(1500.f);
	...
	void __fastcall buff_gain(game_object* object, buff_instance* buff)
	{
		...
		//g_sdk->log_console( "[+] %s gained buff %s | %d", object->get_char_name().c_str(), buff->get_name().c_str(), buff->get_hash());
	}
