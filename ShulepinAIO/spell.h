...
namespace script
{
    ...
    public:
        ...
        [[nodiscard]] bool is_enough_mana_pct(float value);
        [[nodiscard]] bool is_charging();
        [[nodiscard]] bool is_in_range(game_object* target) const;
        ...
        [[nodiscard]] pred_sdk::pred_data get_spell_farm(const int aoe_hits, const spell_farm_flag flags, const dmg_sdk::damage_type damage_type) const;
        [[nodiscard]] pred_sdk::pred_data get_prediction(game_object* target) const;
        [[nodiscard]] pred_sdk::pred_data get_prediction(game_object* target, int hitchance, float range) const;
        [[nodiscard]] pred_sdk::pred_data get_prediction(game_object* target, int hitchance, float range, const std::vector<pred_sdk::hit_type>& hit_type) const;
        [[nodiscard]] pred_sdk::pred_data get_prediction(game_object* target, const pred_sdk::spell_data& pred_input) const;
        [[nodiscard]] int get_collision_count(game_object* target, const int hitchance, const float range) const;
        ...
        [[nodiscard]] float get_travel_time(const math::vector3 position) const;
