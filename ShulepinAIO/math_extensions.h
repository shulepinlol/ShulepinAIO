...
namespace script
{
	...
	public:
		...
		vector2 operator+(const float& flt)  const
		{
			return vector2{ this->x + flt, this->y + flt };
		}
		...
		vector2 operator*(float flt) const
		{
			return vector2{ this->x * flt, this->y * flt };
		}
		...
		float angle_between(vector2 const& v2) const
		{
			if (*this == v2)
				return 0.f;
	...
	public:
		...
		{
			return std::string("vector3{ " + std::to_string(this->x) + "f, " + std::to_string(this->y) + "f, " + std::to_string(this->z) + "f }");
		}
	};
