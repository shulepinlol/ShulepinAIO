...
namespace arena_helper
{
	...
	void trinket_logic()
	{
		...
		for (const auto& hero : heroes)
		{
			...
			const auto cast_position = info.position; //player->get_position().extended(info.position, 350.f / 2.f);
			...
			if (!hero->is_visible() &&
				...
				trinket->is_ready() &&
				cast_position.distance(player->get_position()) < trinket->get_range())
			{
